...
export default function PuzzleEditor({ puzzleDoc, modalVisible, confirmHandler, cancelHandler }) {
  ...
  async function pushNewPuzzle() {
    ...
    try {
      ...
      if (puzzleDocId) {
        await updatePuzzleInDB(puzzleDocId, [quiz1, quiz2, quiz3, quiz4, quiz5]);
      } else {
        await addPuzzleToDB([quiz1, quiz2, quiz3, quiz4, quiz5], auth.currentUser.uid);
      }
  ...
  return (
    <Modal visible={modalVisible} animationType='slide'>
      <View style={styles.container}>
        <Text>quiz 1:</Text>
        <TextInput autoCapitalize='characters' value={quiz1} onChangeText={(text)=>{setQuiz1(text)}} />
        <Text>quiz 2:</Text>
        <TextInput autoCapitalize='characters' value={quiz2} onChangeText={(text)=>{setQuiz2(text)}} />
        <Text>quiz 3:</Text>
        <TextInput autoCapitalize='characters' value={quiz3} onChangeText={(text)=>{setQuiz3(text)}} />
        <Text>quiz 4:</Text>
